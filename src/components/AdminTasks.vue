  ...
  <div>
    <v-layout wrap>
      <v-flex xs12 sm12 md4 mt-3 pl-4>
        <v-toolbar-title>{{ $t('tasks.TITLE') }}</v-toolbar-title>
      </v-flex>
                ...
                <v-layout wrap>
                  ...
                  <v-flex xs12>
                    <v-text-field
                      ...
                      :label="$t('tasks.headers.TASK_NAME')"
                      :data-vv-as="$t('tasks.headers.TASK_NAME')"
                  ...
                  <v-flex xs12>
                    <v-text-field
                      ...
                      :label="$t('tasks.headers.TASK_DESCRIPTION')"
                      :data-vv-as="$t('tasks.headers.TASK_DESCRIPTION')"
                      :error="errors.has('taskDescription')"
                      :error-messages="errors.collect('taskDescription')"
                      v-validate.disable="'required'"
                      autocomplete="off"
                    ></v-text-field>
                  </v-flex>
...
export default {
  metaInfo() {
    return {
      ...
      titleTemplate: `${this.$t('tasks.TITLE')} - %s`
    }
  },
  data() {
    return {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$i18n.t('tasks.headers.TASK_NAME'),
          align: 'left',
          sortable: true,
          value: 'taskName'
        },
        {
          text: this.$i18n.t('tasks.headers.TASK_DESCRIPTION'),
          align: 'left',
          sortable: true,
    ...
    items() {
      return this.$store.state.adminTasks.tasks
    },
    totalItems() {
      return this.$store.state.adminTasks.totalTasks
    }
  },
  watch: {
    dialog(value) {
  ...
  methods: {
    ...mapActions(['getTasks', 'editTask', 'saveTask', 'deleteTask']),
    getFormat(date) {
    ...
    async deleteItem(item) {
      try {
        ...
        if (response) {
          this.dataTableLoading = true
          await this.deleteTask(item._id)
          await this.getTasks(
            buildPayloadPagination(this.pagination, this.buildSearch())
          )
          this.dataTableLoading = false
      ...
      try {
        ...
        if (valid) {
          ...
          if (this.editedItem._id) {
            await this.editTask(this.editedItem)
            await this.getTasks(
              buildPayloadPagination(this.pagination, this.buildSearch())
            )
            this.dataTableLoading = false
